    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function($query) {
                return '<a class="badge badge-primary" href="'.route("admin.blog.edit", $query->id).'" role="button">Edit</a>
                <form action="'.route("admin.blog.destroy", $query->id ).'" method="POST">
                    '.csrf_field().'
                    '.method_field("DELETE").'
            ...
            ->addColumn('created_at', function($query) {
                return date("M j, Y", strtotime($query->created_at));
            })
