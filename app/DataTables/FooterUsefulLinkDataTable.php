    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function($query) {
                return '<a class="badge badge-primary" href="'.route("admin.footer-useful-link.edit", $query->id).'" role="button">Edit</a>
                <form action="'.route("admin.footer-useful-link.destroy", $query->id ).'" method="POST">
                    '.csrf_field().'
                    '.method_field("DELETE").'
